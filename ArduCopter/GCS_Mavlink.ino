...
void GCS_MAVLINK::handleMessage(mavlink_message_t* msg)
{
    ...
    {
               gcs_send_text_P(SEVERITY_LOW,PSTR("data stream requested"));
        // decode
        ...
        {
           gcs_send_text_P(SEVERITY_LOW,PSTR("Broken"));
           gcs_send_text_fmt(PSTR("target_system: %d target_component %d"),packet.target_system,packet.target_component);
            break;
        }
   ...
   //     send_text(SEVERITY_LOW,PSTR("command received: "));
    ...
    case MAVLINK_MSG_ID_SET_MODE:      //11
    {
        ...
        if (!(packet.base_mode & MAV_MODE_FLAG_CUSTOM_MODE_ENABLED)) {
             gcs_send_text_P(SEVERITY_LOW,PSTR("I did get kicked out"));
            // we ignore base_mode as there is no sane way to map
            // from that bitmap to a APM flight mode. We rely on
            // custom_mode instead.
            break;
        }
       // gcs_send_text_P(SEVERITY_LOW,PSTR("I did not get kicked out"));
    ...
    {
        gcs_send_text_P(SEVERITY_LOW,PSTR("waypoint request list"));
        // decode
    ...
    case MAVLINK_MSG_ID_RC_CHANNELS_OVERRIDE: //70
    {
        ...
        v[0] = (int16_t)packet.chan1_raw;
        v[1] = (int16_t)packet.chan2_raw;
        v[2] = (int16_t)packet.chan3_raw;
        v[3] = (int16_t)packet.chan4_raw;
        v[4] = (int16_t)packet.chan5_raw;
        v[5] = (int16_t)packet.chan6_raw;
        v[6] = (int16_t)packet.chan7_raw;
        v[7] = (int16_t)packet.chan8_raw;
       // gcs_send_text_fmt(PSTR("Channels: %d %d %d %d %d %d  %d %d"),v[0],v[1],v[2],v[3],v[4],v[5],v[6],v[7]);
        //gcs_send_text_fmt(PSTR("Channels: %u %u %u %u %u %u  %u %u"),packet.chan1_raw,packet.chan2_raw,packet.chan3_raw,packet.chan4_raw,packet.chan5_raw,packet.chan6_raw,packet.chan7_raw,packet.chan8_raw);
        APM_RC.setHIL(v);
        break;
    }
    
#if ARM_ENABLED == ENABLED
    case MAVLINK_MSG_ID_ARM_CTRL_PWM:
    {
      gcs_send_text_P(SEVERITY_LOW,PSTR("I received arm message"));
